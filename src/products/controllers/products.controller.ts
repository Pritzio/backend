...
export class ProductsController {
  ...
  @ApiResponse({ status: 400, description: 'Bad request - Product code already exists' })
  ...
  @ApiQuery({ name: 'search', required: false, type: String, description: 'Search term for name, description, or code' })
  ...
  @ApiQuery({ name: 'type', required: false, enum: ['physical', 'digital', 'service', 'subscription'], description: 'Filter by product type' })
  @ApiQuery({ name: 'status', required: false, enum: ['active', 'inactive', 'discontinued', 'out_of_stock', 'coming_soon'], description: 'Filter by status' })
  @ApiQuery({ name: 'condition', required: false, enum: ['new', 'used', 'refurbished', 'open_box'], description: 'Filter by condition' })
  @ApiQuery({ name: 'hasWarranty', required: false, type: Boolean, description: 'Filter by warranty availability' })
  @ApiQuery({ name: 'hasDimensions', required: false, type: Boolean, description: 'Filter by dimensions availability' })
  @ApiQuery({ name: 'hasWeight', required: false, type: Boolean, description: 'Filter by weight availability' })
  @ApiQuery({ name: 'minWeight', required: false, type: Number, description: 'Minimum weight filter' })
  @ApiQuery({ name: 'maxWeight', required: false, type: Number, description: 'Maximum weight filter' })
  @ApiQuery({ name: 'minWarranty', required: false, type: Number, description: 'Minimum warranty months' })
  @ApiQuery({ name: 'maxWarranty', required: false, type: Number, description: 'Maximum warranty months' })
  @ApiQuery({ name: 'tags', required: false, type: String, description: 'Comma-separated tags to filter by' })
  @ApiQuery({ name: 'features', required: false, type: String, description: 'Comma-separated features to filter by' })
  ...
  @Get('admin')
  @Roles(RoleType.SUPER_ADMIN, RoleType.ADMIN, RoleType.STORE_ADMIN)
  @ApiOperation({ summary: 'Get products for admin management (with additional filters)' })
  @ApiResponse({ status: 200, description: 'Admin products retrieved successfully' })
  @ApiQuery({ name: 'page', required: false, type: Number, description: 'Page number (default: 1)' })
  @ApiQuery({ name: 'limit', required: false, type: Number, description: 'Items per page (default: 20)' })
  ...
  @ApiResponse({ status: 200, description: 'Product status changed successfully' })
  @ApiResponse({ status: 404, description: 'Product not found' })
  @ApiResponse({ status: 403, description: 'Forbidden - Insufficient permissions' })
  @ApiParam({ name: 'id', description: 'Product ID' })
  @ApiBody({ schema: { type: 'object', properties: { status: { type: 'string', enum: ['active', 'inactive', 'discontinued', 'out_of_stock', 'coming_soon'] } } } })
  ...
  @ApiResponse({ status: 200, description: 'Bulk operation completed' })
  @ApiResponse({ status: 403, description: 'Forbidden - Insufficient permissions' })
  @ApiBody({ schema: { type: 'object', properties: { productIds: { type: 'array', items: { type: 'string' } }, data: { type: 'object' } } } })
