...
export class StoreProductsService {
  ...
  async createStoreProductWithBase(
    ...
    this.productMatchingService.updateProductCounts(baseProductId).catch(error => {
      this.logger.error(`Error updating product counts for baseProductId ${baseProductId}:`, error);
    });
  ...
  async findOrCreateBaseProductWithStrictMatching(
    ...
    try {
      this.logger.log(`🆕 Creating individual base product for: "${productName}" (brand: "${brand}")`);
      ...
      const savedBaseProduct = await this.baseProductRepository.save(baseProduct);
      this.logger.log(`✅ Created individual base product: "${savedBaseProduct.name}" (ID: ${savedBaseProduct.id})`);
      
      return savedBaseProduct;
    } catch (error) {
      this.logger.error(`Error creating individual base product for "${productName}":`, error);
  ...
  async createIndividualBaseProduct(
    ...
    try {
      ...
      this.logger.error(`Error in createIndividualBaseProduct for "${productName}":`, error);
  ...
  private normalizeProductName(name: string): string {
    ...
    let normalized = name
      ...
      .replace(/\b(megarollo|mega rollo|mega-rollo|megarrollo)\b/gi, 'Megarollo')
      ...
      .replace(/\.$/, '')
      .replace(/\s+$/, '')
      .replace(/\b(de|del|en|con|para|por|sin|sobre|bajo|entre|hasta|desde|durante|mediante|según|tras|ante|contra)\b/gi, ' ')
      .replace(/\s+/g, ' ')
      .trim();
  ...
  private normalizeMeasurementOrder(name: string): string {
    ...
    const measurementRegex = /\b(\d+(?:\.\d+)?)\s*(m|cm|mm|ml|g|kg|un|pcs?|pack|rollo|rollos)\b/g;
    ...
    const sortedMeasurements = measurements.sort((a, b) => {
      ...
      const unitPriority: { [key: string]: number } = { 'm': 1, 'un': 2, 'pcs': 2, 'rollo': 3, 'rollos': 3, 'pack': 4 };
  ...
  private async findSimilarBaseProducts(normalizedName: string, normalizedBrand?: string | null): Promise<BaseProduct[]> {
    ...
    if (nameWords.length > 0) {
      ...
      const nameConditions = keyWords.map((word, index) => 
        `bp.name ILIKE :word${index}`
      ).join(' OR ');
  ...
  private findBestMatch(normalizedName: string, normalizedBrand: string | null, products: BaseProduct[]): { product: BaseProduct; similarity: number } | null {
    let bestMatch: { product: BaseProduct; similarity: number } | null = null;
    let bestSimilarity = 0;

    for (const product of products) {
      const similarity = this.calculateSimilarity(normalizedName, normalizedBrand, product);
  ...
  private calculateSimilarity(normalizedName: string, normalizedBrand: string | null, product: BaseProduct): number {
    ...
    if (!brandMatch) {
      ...
      const nameSimilarity = this.jaroWinklerSimilarity(normalizedName, product.name);
  ...
  private areBrandsEqual(brand1: string | null, brand2: string | null): boolean {
  ...
  private extractProductComponents(name: string): {
    ...
    // Extract brand
    const brandMatch = normalized.match(/\b(nova|scott|favorita|elite|abolengo|nubelin|home care|merkat|difem pharma)\b/);
    const brand = brandMatch ? brandMatch[1] : '';
    
    // Extract model
    const modelMatch = normalized.match(/\b(evolution|clasica|ultra|gigante|doble hoja|mega|premium|pro|max|plus|xl|xxl)\b/);
  ...
  private calculatePenalties(name1: string, name2: string): number {
    ...
    if (measurements1.length > 0 && measurements2.length > 0) {
      ...
      if (sorted1.length === sorted2.length && sorted1.every((m, i) => m === sorted2[i])) {
        // Same measurements, no penalty
        penalties += 0;
      } else {
        // Check if they have different quantities (different products)
        const quantities1 = measurements1.filter(m => m.includes('un') || m.includes('pcs'));
        const quantities2 = measurements2.filter(m => m.includes('un') || m.includes('pcs'));
        
        if (quantities1.length > 0 && quantities2.length > 0) {
          const q1 = quantities1.map(q => parseInt(q.match(/\d+/)?.[0] || '0')).sort();
          const q2 = quantities2.map(q => parseInt(q.match(/\d+/)?.[0] || '0')).sort();
    ...
    if (models1.length > 0 && models2.length > 0) {
      const hasDifferentModels = !models1.some(m1 => models2.some(m2 => m1 === m2));
    ...
    if (sizes1.length > 0 && sizes2.length > 0) {
      const hasDifferentSizes = !sizes1.some(s1 => sizes2.some(s2 => s1 === s2));
  ...
  private extractModels(name: string): string[] {
    const modelRegex = /\b(ultra|clasica|gigante|xl|xxl|doble\s+hoja|mega|premium|pro|max|plus)\b/g;
    const matches: string[] = [];
    let match;
  ...
  private extractSizes(name: string): string[] {
    const sizeRegex = /\b(xl|xxl|gigante|mega|mini|small|medium|large|extra\s+large)\b/g;
    const matches: string[] = [];
    let match;
  ...
  private areMeasurementsSimilar(measurements1: string[], measurements2: string[]): boolean {
    // Extract numeric values for comparison
    const values1 = measurements1.map(m => parseFloat(m.match(/\d+(?:\.\d+)?/)?.[0] || '0'));
    const values2 = measurements2.map(m => parseFloat(m.match(/\d+(?:\.\d+)?/)?.[0] || '0'));
