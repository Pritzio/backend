...
export class SimilarityController {
  ...
  @ApiOperation({ 
    ...
    description: 'Find products similar to a given product based on name, brand, and specifications'
  })
  @ApiBody({
  ...
  async findSimilarProducts(@Body() body: {
    name: string;
    brand?: string;
    ...
    const similarProducts = await this.productMatchingService.findSimilarProducts({
      name: name.trim(),
      brand,
      specifications,
      threshold,
  ...
  @ApiQuery({ name: 'threshold', description: 'Similarity threshold (0-1)', required: false, default: 0.8 })
  @ApiQuery({ name: 'limit', description: 'Maximum number of results', required: false, default: 10 })
  @ApiResponse({
    status: 200,
