...
export class StoreProductsController {
  ...
  @ApiResponse({
    status: 400,
    description: 'Bad request - Product already exists in store',
  })
  @ApiResponse({
    status: 403,
  ...
  @Get()
  @ApiOperation({
    summary: 'Get all store products with filtering and pagination',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiQuery({
    ...
    required: false,
    type: String,
    description: 'Search term for name, description, store, or product',
  })
  @ApiQuery({
    name: 'storeId',
    required: false,
  ...
  @ApiQuery({
    name: 'scrapingStatus',
    required: false,
    enum: ['pending', 'in_progress', 'completed', 'failed', 'scheduled'],
  ...
  @ApiQuery({
    ...
    required: false,
    type: Boolean,
    description: 'Filter by online price availability',
  })
  @ApiQuery({
    name: 'hasPhysicalPrice',
    required: false,
    type: Boolean,
    description: 'Filter by physical price availability',
  })
  @ApiQuery({
    name: 'isOnSale',
    required: false,
  ...
  @ApiQuery({
    ...
    required: false,
    type: Boolean,
    description: 'Filter by scraping needs',
  })
  @ApiQuery({
    name: 'createdBy',
    required: false,
  ...
  @ApiQuery({
    name: 'lastScrapedAfter',
    required: false,
    type: String,
    description: 'Filter by last scraping date (ISO string)',
  })
  @ApiQuery({
  ...
  async getAllStoreProducts(
    ...
    const filter: IStoreProductFilter = {
      ...
      lastScrapedAfter: lastScrapedAfter
        ? new Date(lastScrapedAfter)
        : undefined,
      lastScrapedBefore: lastScrapedBefore
        ? new Date(lastScrapedBefore)
        : undefined,
    };
  ...
  @ApiOperation({
    summary:
      'Get store products for admin management (with additional filters)',
  })
  @ApiResponse({
    status: 200,
    description: 'Admin store products retrieved successfully',
  })
  @ApiQuery({
    name: 'page',
    required: false,
  ...
  async getStoreProductAnalytics(
    @Request() req: any,
  ): Promise<IStoreProductAnalytics> {
  ...
  @ApiResponse({
    status: 200,
    description: 'Store product status changed successfully',
  })
  @ApiResponse({ status: 404, description: 'Store product not found' })
  @ApiResponse({
    status: 403,
  ...
  async changeStoreProductStatus(
    ...
    return this.storeProductsService.changeStoreProductStatus(
      id,
      status,
      req.user,
    );
  ...
  @ApiResponse({
    status: 200,
    description: 'Scraping result processed successfully',
  })
  @ApiResponse({ status: 404, description: 'Store product not found' })
  @ApiResponse({
    status: 403,
  ...
  async processScrapingResult(
    ...
    return this.storeProductsService.processScrapingResult(
      scrapingResult,
      req.user,
    );
  }
  ...
  async startScraping(
    ...
    const storeProduct = await this.storeProductsService.getStoreProductById(
      id,
      req.user,
    );
  ...
  @ApiBody({
    schema: {
      type: 'object',
      properties: {
        storeProductIds: { type: 'array', items: { type: 'string' } },
      },
    },
  })
  ...
  @ApiOperation({
    summary: 'Compare prices for a product across different stores',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiQuery({
    ...
    required: false,
    type: String,
    description: 'Currency for price comparison (default: USD)',
  })
  async getPriceComparison(
    ...
    const result = await this.storeProductsService.getAllStoreProducts(
      req.user,
      filter,
      1,
      100,
    ...
    if (result.storeProducts.length === 0) {
      throw new NotFoundException(
        `No store products found for product ID '${productId}'`,
      );
    }
    ...
    const stores = result.storeProducts.map((sp) => ({
      ...
      bestPrice:
        sp.onlinePrice && sp.physicalPrice
          ? Math.min(sp.onlinePrice, sp.physicalPrice)
          : sp.onlinePrice || sp.physicalPrice || 0,
